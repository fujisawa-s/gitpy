        ...
        if 'msg' in msg_temp.get('type'):
            ...
            if send_or_recv[0] == '0':
                ...
                if msg_content == news_open_key and news_switch == False:
                    ...
                    wx_inst.send_text(wechar_user, '你已打开新闻阅览,请选择新闻类型(输入数字):[1]热点,[2]社会,[3]国内,[4]国际,[5]娱乐,[6]体育,[7]军事,[8]科技,[9]财经,[10]时尚;发送[关闭新闻]退出')
                    news_switch = True
               ...
               #如果已经开启新闻，那么开始选择类别
                     ...
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('top'))
                elif msg_content == '2' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('shehui'))
                elif msg_content == '3' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('guonei'))
                elif msg_content == '4' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('guoji'))
                elif msg_content == '5' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('yule'))
                elif msg_content == '6' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('tiyu'))
                elif msg_content == '7' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('junshi'))
                elif msg_content == '8' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('keji'))
                elif msg_content == '9' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('caijing'))
                elif msg_content == '10' and news_switch == True:
                     wx_inst.send_text(wechar_user,'%s' % newsReturn.get_type('shishang'))
